// app/api/history/execories/[id]/history/route.ts
...
interface AdminRequisitionItemSimple {
  id: number;
  requisition_id: number;
}

interface AdminInventoryTransaction {
  id: number;
  ...
  reason: string | null;
  created_at: string;
  admin_item_requisition_items: AdminRequisitionItemSimple[] | null;
}
...
export async function GET(
  ...
  try {
    const { data: auditLogs, error: auditError } = await supabase
      .from('admin_item_requisition_audit_log')
      .select('*')
    ...
    let transactions: AdminInventoryTransaction[] = [];
    
    if (itemIds.length > 0) {
      const { data: rawTransactions, error: txError } = await supabase
        .from('admin_item_inventory_transactions')
        .select(`
          id,
          admin_item_id,
          requisition_item_id,
          ...
          reason,
          created_at,
          admin_item_requisition_items(id, requisition_id)
        `)
      ...
      if (txError) {
        ...
        transactions = rawTransactions as unknown as AdminInventoryTransaction[];
      }
    }

    const history: AdminRequisitionHistoryItem[] = [];

    if (auditLogs) {
      history.push(
        ...auditLogs.map((log: AdminAuditLog): AdminRequisitionHistoryItem => ({
          type: 'audit',
    ...
    if (transactions) {
      history.push(
        ...transactions
          .filter((tx) => tx.admin_item_requisition_items && tx.admin_item_requisition_items.length > 0)
          .map((tx): AdminRequisitionHistoryItem => ({
            type: 'transaction',
