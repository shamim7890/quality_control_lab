// @/app/admin-requisitions/new/page.tsx
'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useUser } from '@clerk/nextjs';
import { AdminItemRequisitionFormData, AdminItem, AdminItemRequisitionItemForm } from '@/types/admin-requisition';

export default function NewAdminRequisitionPage() {
    const { isLoaded, isSignedIn, user } = useUser();
    const router = useRouter();
    const [formData, setFormData] = useState<AdminItemRequisitionFormData>({
        requisition_date: new Date().toISOString().split('T')[0],
        department: '',
        requester: user?.fullName || '',
        items: [],
    });
    const [availableItems, setAvailableItems] = useState<AdminItem[]>([]);
    ...
    useEffect(() => {
        if (isLoaded && isSignedIn) {
            ...
            if (role !== 'analyst') {
                router.push('/requisitions/execories');
            }
    ...
    useEffect(() => {
        ...
        async function fetchItems() {
            ...
            if (!response.ok) {
                setError('Failed to load admin items');
                return;
            }
    ...
    const filteredItems = availableItems.filter((item) =>
        ...
        !formData.items.some(formItem => formItem.admin_item_id === item.id)
    );
    ...
    const addItem = (item: AdminItem) => {
        const newItem: AdminItemRequisitionItemForm = {
    ...
    const updateItem = (index: number, field: keyof AdminItemRequisitionItemForm, value: number | string) => {
        setFormData((prev) => {
        ...
        if (field === 'requested_quantity') {
            ...
            const adminItem = availableItems.find(c => c.id === item.admin_item_id);
            
            if (adminItem && typeof value === 'number') {
                if (value > adminItem.quantity) {
                    setValidationErrors(prev => ({
                        ...prev,
                        [index]: `Cannot exceed available quantity (${adminItem.quantity} ${adminItem.unit})`
                    }));
                } else if (value <= 0) {
                    setValidationErrors(prev => ({
                        ...prev,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const response = await fetch('/api/admin-requisitions', {
            method: 'POST',
            ...
            <div className="max-w-5xl mx-auto p-6">
                <div className="bg-gray-800 rounded-lg shadow-2xl border border-gray-700">
                    <div className="bg-gradient-to-r from-purple-600 to-purple-800 p-6 rounded-t-lg">
                        <h1 className="text-3xl font-bold text-white">New Execories Item Requisition</h1>
                        <p className="text-purple-100 mt-2">Request execories items from inventory</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                                    required
                                />
                            </div>
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={handleInputChange}
                                    className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full p-4 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                                />
                                        ...
                                        {filteredItems.map((item) => (
                                            ...
                                            >
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <p className="text-gray-100 font-semibold">{item.item_name}</p>
                                                        <p className="text-sm text-gray-400 mt-1">
                                                            Available: <span className="font-semibold text-green-400">{item.quantity} {item.unit}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                            </li>
                                ...
                                {searchTerm && filteredItems.length === 0 && (
                                    <div className="absolute z-10 w-full mt-2 bg-gray-700 border border-gray-600 rounded-lg p-4 text-gray-400 shadow-lg">
                                        No matching items found or all available items already added
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-3">
                                        ...
                                        >
                                            <div className="flex gap-4 items-start">
                                                <div className="flex-1">
                                                    <p className="font-semibold text-gray-100 mb-1">{item.item_name}</p>
                                                    <div className="flex gap-4 text-sm text-gray-400">
                                                        <span>Unit: <span className="text-gray-300">{item.unit}</span></span>
                                                        <span>Available: <span className="text-green-400 font-semibold">{adminItem?.quantity || 0} {item.unit}</span></span>
                                                    </div>
                                                </div>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    ...
                                                    <input
                                                        ...
                                                        className={`w-full p-2 bg-gray-600 border rounded-lg text-gray-100 focus:outline-none focus:ring-2 transition-all ${
                                                            hasError 
                                                                ...
                                                                : 'border-gray-500 focus:ring-purple-500'
                                                        }`}
                                                        min="0"
                                                        step="0.001"
                                                        max={adminItem?.quantity || 0}
                                                        required
                                                    />
                                                    {hasError && (
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full p-2 bg-gray-600 border border-gray-500 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                                                    />
                                                </div>
                                            </div>
                                        </div>
                                ...
                                {formData.items.length === 0 && (
                                    <div className="bg-gray-700/50 border-2 border-dashed border-gray-600 rounded-lg p-8 text-center">
                                        ...
                                        <p className="text-gray-500 text-sm mt-2">Search and select items above to add them to your requisition</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {formData.items.length > 0 && (
                            <div className="bg-purple-900/20 border border-purple-700 rounded-lg p-4">
                                <h3 className="font-semibold text-purple-300 mb-2">Requisition Summary</h3>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                    <div>
                                        <p className="text-gray-400">Total Items</p>
                                        <p className="text-2xl font-bold text-purple-400">{getTotalItems()}</p>
                                    </div>
                                    <div>
                        ...
                        <div className="flex gap-3 pt-6 border-t border-gray-700">
                            ...
                            <button
                                ...
                                className="flex-1 px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-800"
                            >
                                {loading ? (
