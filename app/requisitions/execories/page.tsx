// @/app/admin-requisitions/page.tsx
'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useUser } from '@clerk/nextjs';
import { AdminItemRequisition } from '@/types/admin-requisition';
...
interface RequisitionWithItems extends AdminItemRequisition {
...
export default function AdminRequisitionsPage() {
    ...
    async function fetchRequisitions() {
        setLoading(true);
        setError(null);
        const response = await fetch(`/api/admin-requisitions?status=${statusFilter}`);
        if (!response.ok) {
    ...
    async function handleAction(
        ...
        const response = await fetch(`/api/admin-requisitions/${requisitionId}/approve`, {
            method: 'PUT',
    ...
    // @/app/admin-requisitions/page.tsx (continued)
        ...
        <div className="min-h-screen bg-gray-900 text-gray-100 p-6">
            <div className="max-w-7xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-3xl font-bold">Execories Items Requisitions</h1>
                    <div className="flex gap-3">
                        {isAnalyst && (
                            <button 
                                onClick={() => router.push('/admin-requisitions/new')} 
                                className="px-4 py-2 bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors font-semibold"
                            >
                                + New Requisition
                            </button>
                        )}
                                            ...
                                            {req.items.map((item) => (
                                                <div key={item.id} className="flex items-center gap-3">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-24 px-2 py-1 bg-gray-600 border border-gray-500 rounded text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                                                    />
