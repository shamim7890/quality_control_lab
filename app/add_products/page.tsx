...
export default function AddProductPage() {
  ...
  const [newProduct, setNewProduct] = useState<Product>({ name: '', unit: '', uses: '' });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSuccess(true);
        setNewProduct({ name: '', unit: '', uses: '' });
        setTimeout(() => {
          router.push('/add_products/history');
        }, 2000);
      } else {
        // Handle API errors (e.g., validation failures from route.ts)
  ...
  const isFormValid = newProduct.name.trim() && newProduct.unit.trim();
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-slate-800 mb-2">Add New Product</h1>
                <p className="text-slate-600 text-lg">Expand your inventory with new products</p>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white/80 backdrop-blur-sm shadow-xl rounded-2xl border border-white/50 overflow-hidden">
                ...
                <div className="p-8">
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4 mb-6 animate-in slide-in-from-top-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-green-700 text-sm">Product created successfully. Redirecting...</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-6">
                    ...
                    {/* Unit */}
                    <div className="space-y-2">
                      <label htmlFor="unit" className="block text-sm font-semibold text-slate-700">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onBlur={() => setFocusedField(null)}
                          className={`w-full px-4 py-3 rounded-xl border-2 transition-all duration-200 bg-white/50 backdrop-blur-sm appearance-none cursor-pointer
                            ${focusedField === 'unit' 
                              ? 'border-blue-500 ring-4 ring-blue-100 shadow-lg' 
                              : 'border-slate-200 hover:border-slate-300'
                            }
                            focus:outline-none text-slate-800 ${!newProduct.unit ? 'text-slate-400' : ''}`}
                        >
                    ...
                    {/* Uses */}
                    <div className="space-y-2">
                      <label htmlFor="uses" className="block text-sm font-semibold text-slate-700">
                        Product Uses & Applications
                        <span className="text-slate-500 font-normal ml-1">(Optional)</span>
                      </label>
                      <textarea
                        id="uses"
                        placeholder="Describe how this product is used, its applications, or any relevant details..."
                        value={newProduct.uses || ''}
                        onChange={(e) => setNewProduct({ ...newProduct, uses: e.target.value })}
                        onFocus={() => setFocusedField('uses')}
                        onBlur={() => setFocusedField(null)}
                ...
                <div className="p-6">
                  <div className="space-y-4 text-sm">
                    <div className="flex gap-3">
                      ...
                      <p className="text-slate-700">Use clear, descriptive names that are easy to search and identify</p>
                    </div>
                    <div className="flex gap-3">
                      <div className="w-2 h-2 bg-green-500 rounded-full mt-2 flex-shrink-0"></div>
                      <p className="text-slate-700">Choose the most appropriate unit from the dropdown options</p>
                    </div>
                    <div className="flex gap-3">
                      <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                      <p className="text-slate-700">Include detailed uses to improve inventory management</p>
                    </div>
                    <div className="flex gap-3">
                      <div className="w-2 h-2 bg-red-500 rounded-full mt-2 flex-shrink-0"></div>
                      <p className="text-slate-700">Fields marked with * are required for submission</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="p-6">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <CheckCircle className={`w-4 h-4 ${newProduct.name ? 'text-green-500' : 'text-slate-300'}`} />
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-700">Unit</span>
                      <CheckCircle className={`w-4 h-4 ${newProduct.unit ? 'text-green-500' : 'text-slate-300'}`} />
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-700">Uses (Optional)</span>
                      <CheckCircle className={`w-4 h-4 ${newProduct.uses ? 'text-green-500' : 'text-slate-300'}`} />
                    </div>
                    <div className="pt-2 border-t border-slate-200">
                      <div className="text-xs text-slate-600 mb-2">Completion: {Math.round(((newProduct.name ? 1 : 0) + (newProduct.unit ? 1 : 0)) / 2 * 100)}%</div>
                      <div className="w-full bg-slate-200 rounded-full h-2">
                        <div 
                          className="bg-gradient-to-r from-blue-500 to-indigo-500 h-2 rounded-full transition-all duration-300"
                          style={{ width: `${((newProduct.name ? 1 : 0) + (newProduct.unit ? 1 : 0)) / 2 * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
