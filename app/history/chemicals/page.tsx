// app/history/chemicals/page.tsx
...
async function getRequisitions(): Promise<Requisition[]> {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/history/chemicals`,
      {
        headers: {
...
export default async function RequisitionsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="container mx-auto px-6 py-8">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Chemical Requisitions</h1>
              <p className="text-gray-600 mt-2">
                View and manage all chemical requisition requests
              </p>
            </div>
      ...
      {/* Main Content */}
      <div className="container mx-auto px-6 py-8">
        <RequisitionsList initialRequisitions={requisitions} />
      </div>
    </div>
  );
}